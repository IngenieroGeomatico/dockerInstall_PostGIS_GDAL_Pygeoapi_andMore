...
# Calcular la versión a partir de la variable de entorno PG_Version
PG_Version_int=$(echo "$PG_Version" | cut -d'.' -f1)
export PG_Version_int
...
su - postgres -c "/var/lib/postgresql/bin/pg_ctl -D /var/lib/postgresql/data stop || echo 'No se encontró ninguna instancia en ejecución.'"



# Iniciar PostgreSQL con un tiempo de espera mayor
echo "Iniciando el servicio de PostgreSQL..."
su - postgres -c "/var/lib/postgresql/bin/pg_ctl -D /var/lib/postgresql/data -l /var/lib/postgresql/logfile start -t 120 "
...
i=1
while [ $i -le 20 ]; do
    if su - postgres -c '/var/lib/postgresql/bin/psql -c "SELECT 1;" ' > /dev/null 2>&1; then
